...
(function () {
  ...
  function loadResources(cb) {
    fetch("/api/resources").then(function (r) { return r.json(); }).then(function (d) { cb(d.resources || []); }).catch(function () { cb([]); });
  }
  function openResources() {
    var ov = document.getElementById("resDrawer"); if (!ov) return;
    ov.hidden = false;
    var close = document.getElementById("resClose"); if (close) close.focus();
    if (!_resLoaded) { _resLoaded = true; loadResources(renderResourceList); }
  }
